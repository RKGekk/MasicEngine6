...
TiXmlElement::TiXmlElement(const TiXmlElement& copy) : TiXmlNode(TiXmlNode::NodeType::TINYXML_ELEMENT) {
	firstChild = lastChild = 0;
	copy.CopyTo(this);
}
...
int TiXmlElement::QueryBoolAttribute(const char* name, bool* bval) const {
	...
	if (StringEqual(node->Value(), "true", true, TiXmlEncoding::TIXML_ENCODING_UNKNOWN) || StringEqual(node->Value(), "yes", true, TiXmlEncoding::TIXML_ENCODING_UNKNOWN) || StringEqual(node->Value(), "1", true, TiXmlEncoding::TIXML_ENCODING_UNKNOWN)) {
		*bval = true;
		result = TIXML_SUCCESS;
	}
	else if (StringEqual(node->Value(), "false", true, TiXmlEncoding::TIXML_ENCODING_UNKNOWN) || StringEqual(node->Value(), "no", true, TiXmlEncoding::TIXML_ENCODING_UNKNOWN) || StringEqual(node->Value(), "0", true, TiXmlEncoding::TIXML_ENCODING_UNKNOWN)) {
		*bval = false;
		result = TIXML_SUCCESS;
	}
	return result;
...
TiXmlDeclaration::TiXmlDeclaration(const char* _version, const char* _encoding, const char* _standalone) : TiXmlNode(TiXmlNode::NodeType::TINYXML_DECLARATION) {
	version = _version;
	encoding = _encoding;
...
TiXmlDeclaration::TiXmlDeclaration(const TiXmlDeclaration& copy) : TiXmlNode(TiXmlNode::NodeType::TINYXML_DECLARATION) {
	copy.CopyTo(this);
}
