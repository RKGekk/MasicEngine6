...
class StatsRenderComponent;

class D3D11StatPipeline : public SceneNode {
private:
	std::shared_ptr<VertexShader> m_vs;
	std::shared_ptr<PixelShader> m_ps;

public:
	D3D11StatPipeline(int mesh_id, StatsRenderComponent& data, const DirectX::XMFLOAT4X4* pMatrix);
	D3D11StatPipeline(int mesh_id, StatsRenderComponent& data, DirectX::FXMMATRIX to, DirectX::CXMMATRIX from, bool calulate_from = false);
	virtual ~D3D11StatPipeline();
