...
#include "../physics/particle_geo_gravity.h"
...
void ParticleForceGeneratorComponent::VPostInit() {
    ...
    if (m_force_generator_type_name == "GeoGravity" && pTransformComponent) {
        m_force_generator = std::make_shared<ParticleGeoGravity>(pTransformComponent->GetPosition(), m_gravity);
    }
